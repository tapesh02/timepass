...
const FilterNav = ({
  ...
  return <>
       <div className={styles.filtermain}>
             ...
             {
              showGenresFilter?  
              <Grow  in = {showGenresFilter}
              style={{ transformOrigin: '0 0 0' }}
              {...(showGenresFilter ? { timeout: 1000 } : {})} > 
              <div>
              { selectedGenres?.map((genre) => (
                                                 <Chip 
                                                 variant="outlined" 
                                                 size="small" 
                                                 style = {{marginTop : "3px", marginLeft: "2px", marginRight: "2px", backgroundColor: "whitesmoke" ,borderColor: "none", color: "green"}}
                                                 clickable
                                                 label = {genre.name}
                                                 key={ genre.id}
                                 ...
                                 {  genres?.map((genre) => (
                                       <Chip 
                                                 variant="outlined" 
                                                 size="small" 
                                                 style = {{marginTop : "3px", marginLeft: "2px", marginRight: "2px", backgroundColor: "whitesmoke", borderColor: "none"}}
                                                 clickable
                                                 label = {genre.name}
                                                 key={ genre.id}
                                                 deleteIcon={<DoneIcon />} 
